...
nsample=1000
Ps1 = as.data.frame(runif(min=0, max=1, n=nsample))
colnames(Ps1) = "p"

Ps2 = as.data.frame(runif(min=0, max=1, n=nsample))
colnames(Ps2) = "p"
...
#Set up data frame
res_init = rep(0,times=nsim)
res <- data.frame(Iteration = res_init, LastYearDep = res_init)
res_nitrogen <- res_init
...
for (i in 1:nsim) {
  ...
  res$Iteration[i] = i
  res$LastYearDep[i] = tmp[100,2]
  res_nitrogen[i] = res$LastYearDep[i]
} 

#Give our results to sensitivity structure
dep_vector = res$LastYearDep
sens_micro = sensitivity::tell(sens_micro, res_nitrogen)

# look at results
sens_micro$S
sens_micro$T

tmp = cbind.data.frame(sens_micro$X, nitrogen=sens_micro$y)
ggplot(tmp, aes(c1, nitrogen))+
  geom_point()+
